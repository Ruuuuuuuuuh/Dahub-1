<template>
  <v-container fluid style="width: 90%; height: 90%">
    <v-container class="fullwidth">
      ...
      <v-list-item dense>
        <v-row v-if="assets != null">
          ...
          <v-col v-if="assets[selected_payment].recentBallanceChange != null">
            <overline
              v-if="
                assets[selected_payment].recentBallanceChange.way === 'increase'
              "
            >
            ...
            <overline
              v-if="
                assets[selected_payment].recentBallanceChange.way === 'decrease'
              "
            >
              - {{ assets[selected_payment].recentBallanceChange.value }}
            </overline>
      ...
      <v-spacer/>
      <v-container v-if="createdRequests!=null && screenMode === 0" class="mt-5">
        <h3>Открытые заявки</h3>
        <v-spacer />
        <requestFrontFace
          :request_number="createdRequests.requestNumber"
          :request_date="createdRequests.requestDate"
          :request_time="createdRequests.requestTime"
          :request_status="this.pendingRequestMessage[createdRequests.status]"
          :request_amount="createdRequests.amount"
          :request_currency="createdRequests.currency"
          :request_type="'in'"
        ></requestFrontFace>
      </v-container>
    </v-container>
...
import requestFrontFace from "../../components/requestFrontFace"
import EventBus from "../../eventbus";

export default {
  components: { HistoryTab, requestFrontFace },
  data: () => ({
    selected_payment: 0,
    historyData: null,
  ...
  computed: {
    ...
    createdRequests(){
      return this.$store.state.requestUser.createdRequests[0];
    }
  },
