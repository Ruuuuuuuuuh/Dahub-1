...
<div class="section-main">
    ...
    <div class="text-block">
        @if ($order->currency == 'TON')
            ...
            <p class="w-100 mt-2">Адрес: <a class="wallet-link copy-link copy-link-small" data-toggle="popover"
                                            data-placement="bottom" data-content="Ссылка скопирована в буфер обмена."
                                            data-original-title="" title="">
                    <span>💎&nbsp;{{$order->payment_details}}</span>
                </a></p>
            <p class="w-100 mt-2">Примечание (мемо): <a class="wallet-link copy-link copy-link-small"
            ...
            <p class="mt-3">Как только средства будут зачислены, вы получите уведомление. Среднее время зачисления
                средств – одна минута.</p>

        @else
            <p>Переведите {{$order->amount}} {{$order->currency}}
                по @if (App\Models\Payment::where('title', $order->payment)->firstOrFail()->crypto)
                    адресу
                @else
                    номеру карты
                @endif:</p>
    ...
    <div class="footer">
        @if ($order->currency == 'TON')
            <a href="{{ "ton://transfer/" . $order->payment_details . "?amount=" . ($order->amount * 1000000000) . "&text=" . $order->comment}}"
               class="button button-blue button-accept">Открыть Toncoin Кошелек</a>
        @else
